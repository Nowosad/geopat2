...
int* convert_queue_to_table(struct fifo* fifo)
{
	if(!fifo)
		return NULL;
	int i=0;
	int* table=malloc(fifo->length*sizeof(int));
	struct node* next=fifo->first;
...
int* convert_queue_to_edges(struct fifo* fifo)
{
	if(!fifo)
		return NULL;
	int i=0;
	...
	while(next) {
		...
		next=next->next_node;
		if(i>fifo->length)
			G_fatal_error("Cannot convert queue to edges %d %d", i, fifo->length);
		i++;
	}
	return table;
}
